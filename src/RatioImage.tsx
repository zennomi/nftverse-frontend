...
const ImageMaterialImpl = /* @__PURE__ */ shaderMaterial(
  ...
  `,
    ...
    void main() {
      ...
      #include <${Number(React.version) >= 154 ? 'colorspace_fragment' : 'encodings_fragment'}>
    }
  `
)
...
const ImageBase: ForwardRefComponent<Omit<ImageProps, 'url'>, THREE.Mesh> = /* @__PURE__ */ React.forwardRef(
    (
        ...
        const ratio = [texture!.image.width / texture!.image.height * scale, scale]
        const ref = React.useRef<THREE.Mesh>(null!)
        const size = useThree((state) => state.size)
        const planeBounds = ratio
        ...
        return (
            <mesh ref={ref} scale={[ratio[0], ratio[1], 1]} {...props}>
                <boxGeometry args={[1, 1, depth, segments, segments, segments]} />
                <imageMaterial
                    color={color}
                    map={texture!}
                    zoom={zoom}
...
export const RatioImage: ForwardRefComponent<ImageProps, THREE.Mesh> = /* @__PURE__ */ React.forwardRef<
    THREE.Mesh,
    ImageProps
>((props, ref) => {
